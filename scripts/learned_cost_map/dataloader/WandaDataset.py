...
class DatasetBaseWanda(Dataset):
    '''
    ...
    def __init__(self, \
        ...
        super(DatasetBaseWanda, self).__init__()
        ...
        if 'cost' in self.datatypelist:
            self.costlist = self.loadDataFromFile(self.trajlist, 'cost/float.npy')
    ...
    def getDataPath(self, trajstr, framestrlist, datatype):
        ...
        for framestr in framestrlist: 
            ...
            if datatype == 'imgc':
                datapathlist.append(trajstr + '/image_left/' + framestr + '.png')
            if datatype == 'disp0':
                raise NotImplementedError("No depth in this dataset")
...
def wanda_data_transform(sample, augment_data=False):
...
if __name__ == '__main__':
    ...
    dataset = DatasetBaseWanda(train_split,
